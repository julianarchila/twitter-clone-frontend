...
const TweetMenu: React.FC<Props> = (props) => {
  ...
  const handleRemove = async () => {
    const confirm: boolean = window.confirm(
      `Are you sure you want delete this tweet: ${tweet.content} ?`
    );
    if (confirm) {
  ...
  return (
    <div className="tweet-menu__items">
      <div className="tweet-menu__item" onClick={handleCopy}>
        <FaCopy className="icon" />
        Copy
      </div>
      {tweet.user.username === auth.user.username ? (
        <div className="tweet-menu__item" onClick={handleRemove}>
          <FaTrash className="icon" />
          Delete
        </div>
      ) : (
        <div className="tweet-menu__item" onClick={handleReport}>
